...
def grad_rosen(x):
  return nd.Gradient(rosenbrock)(x)

def hessian_rosen(x):
  return nd.Hessian(rosenbrock)(x)
...
def gold_stein(f, g, xk, alpha, pk, c):
  return (f(xk) + (1-c)*alpha*np.dot(g(xk), pk) <= f(xk+alpha*pk) ) and (f(xk+alpha*pk) <= f(xk)+ c*alpha*np.dot(g(xk), pk))
...
if __name__ == '__main__':
  ...
  print "Steepest Descent terminated in {} iterations, x = {}, f(x) = {}, \
    time elapsed {}, time per iter {}".format(n_iter, x, rosenbrock(x), end-start, (end-start)/n_iter)
  ...
  print "Conjugate Gradient Method terminated in {} iterations, x = {}, f(x) = {}, \
    time elapsed {}, time per iter {}".format(n_iter, x, rosenbrock(x), end-start, (end-start)/n_iter)
  ...
  print "Newton\'s Method terminated in {} iterations, x = {}, f(x) = {}, \
    time elapsed {}, time per iter {}".format(n_iter, x, rosenbrock(x), end-start, (end-start)/n_iter)
  ...
  print "BFGS terminated in {} iterations, x = {}, f(x) = {}, \
    time elapsed {}, time per iter {}".format(n_iter, x, rosenbrock(x), end-start, (end-start)/n_iter);
  ...
  print "l-BFGS terminated in {} iterations, x = {}, f(x) = {}, \
    time elapsed {}, time per iter {}".format(n_iter, x, rosenbrock(x), end-start, (end-start)/n_iter);
